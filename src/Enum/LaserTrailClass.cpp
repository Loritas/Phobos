...
bool LaserTrailClass::Draw(CoordStruct location)
{
    ...
    {
        if (this->Type->IgnoreVertical ? (abs(location.X - this->LastLocation.Get().X) > 16 || abs(location.Y - this->LastLocation.Get().Y) > 16) : true)
        {
            ...
            pLaser->IsHouseColor = this->Type->IsHouseColor; // TODO check why OG had always true here
